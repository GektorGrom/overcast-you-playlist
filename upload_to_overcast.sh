echo Log In...
curl -o /dev/null -L -d "then=podcasts" -d "email=<OVERCAST_USER_EMAIL>" -d "password=<OVERCAST_PASSWORD>" -X POST -c overcast_cookies https://overcast.fm/login || exit 1
...
curl -L -F "key=$key" -X POST -b overcast_cookies https://overcast.fm/podcasts/upload_succeeded/ || exit
